...
@section('title', 'Blog | ' . $page_name . ' list')
        ...
        <div class="card-body">
          <table class="table table-bordered table-striped">
            ...
            <tbody>
                ...
                @php
                    $categoryArray = is_array($blog->category) ? $blog->category : json_decode($blog->category ?? '[]', true);
                    $services = collect(Service())->keyBy('id');
                @endphp
                    <tr>
                        ...
                        <td>
                             @if(!empty($categoryArray))
                                  {{ implode(', ', collect($categoryArray)->map(fn($id) => $services[$id]->name ?? '')->filter()->toArray()) }}
                              @else
                                  -
                              @endif
                        </td>
                        ...
                        <td>
                            @if($blog->featured_image)
                                <img src="{{ asset( $blog->featured_image) }}" width="60">
                            @else
                                N/A
                            @endif
                        </td>
