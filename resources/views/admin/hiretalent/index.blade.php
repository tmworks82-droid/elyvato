      ...
      @endif
        ...
        <div class="card-header">
          ...
          @if(auth()->user()->hasPermission($permission))
            <a class="float-right" href="{{ route('admin.createhire.talent') }}">
          ...
          <table class="table table-bordered table-striped">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($talent->icon)
                            <img src="{{ url($talent->icon) }}" alt=" Image" style="width: 30px;">
                        @else
                            No Image
                        @endif
                    </td>
                    <td>
                        @if($talent->image)
                            <img src="{{ url($talent->image) }}" alt=" Image" style="width: 30px;">
                        @else
                            No Image
                        @endif
                    </td>
                    <td>@if($talent->is_active==1) Active @else Inactive @endif</td>
                    <td>{{ $talent->created_at->format('d-m-Y H:i:s') }}</td>
                    <td> 
                        <a href="{{ route('admin.edit.talent',$talent->id) }}" class="btn btn-sm btn-warning">Edit</a>
                        <form action="{{ route('admin.destroy.talent', $talent->id) }}" method="POST" style="display: inline;">
                            @csrf
