              ...
              <form method="GET" action="{{route('projects')}}" class="flex space-x-4">
                ...
                <select name="sub_service_id" class="border p-2 rounded form-control">
                    ...
                    @foreach($sub_services as $sub_service)
                        <option value="{{ $sub_service->id }}" {{ request('sub_service_id') == $sub_service->id ? 'selected' : '' }}>
                            {{ $sub_service->name }}
                        </option>
                    @endforeach 
                </select>
                    ...
                    @foreach ($projects as $project)
                      ...
                      <tr>
                          ...
                          <td>
                              ...
                              <a href="{{url('projects-details', $project->id)}}" class="project-link" data-type="project" data-id="{{$project->id}}"> <span class="badge badge-info">{{GetUser($project->created_by)->name ?? GetUser($project->created_by)->username}}  </span> </a>
                              @else
                              <a href="{{url('projects-details', $project->id)}}"> <span class="badge badge-info">{{GetUser($project->booking->created_by)->name ?? GetUser($project->booking->created_by)->username}}</span> </a>
                              @endif
                              ({{GetUser($project->created_by)->mobile}})
                              </td>

                          <td> 
                          @if($project->is_visited=='no')
                          
                          <a href="{{url('projects-details', $project->id)}}" target="_blank" class="project-link" data-type="project" data-id="{{$project->id}}">
                            @if(!empty($project->booking->sow))
                                {{$project->booking->sow->service->name ?? 'no service'}} -> {{$project->booking->sow->subservice->name ?? 'N/A'}} <br> {!! trimWords($project->booking->sow->description) !!}
                                @else
                                {{$project->booking->hireTalent->name}}
                             @endif
                            </a>
                            @else 
                             <a href="{{url('projects-details', $project->id)}}" target="_blank" class="project_link_visited">
                           @if(!empty($project->booking->sow))
                                {{$project->booking->sow->service->name}} -> {{$project->booking->sow->subservice->name}} <br> {!! trimWords($project->booking->sow->description) !!}
                          ...
                          <td>
                            ...
                            <select name="project_status" class="project-status" data-id="{{ $project->id }}">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <select name="submitted_for_review" id="submitted_for_review" class="form-control" required>
