                        ...
                        <div class="card">
                            <div class="card-header">
                                ...
                                @if (auth()->user()->hasPermission('create_' . $permission))
                                    <a class="float-right" href="{{ url('create-statement') }}">
                                        <button type="button" class="btn btn-block btn-primary btn-sm ml-2">Add
                                            New</button>
                                    </a>
                                @endif
                            </div>
                                    ...
                                    <tbody>
                                        @forelse($statements as $statement)
                                            ...
                                            <tr>
                                                ...
                                                <td>₹{{ number_format($statement->min_price, 2) }}</td>
                                                <td>₹{{ number_format($statement->max_price, 2) }}</td>
                                                <td>₹{{ number_format($statement->offer_price, 2) }}</td>
                            ...
                            @endif
                                                        ...
                                                        @foreach ($statement->allFiles as $files)
                                                            @if ($files->file_type == 'image')
                                                                <div class="media-wrapper" data-id="{{ $files->id }}">
                                                                    <a href="{{ url($files->image_path) }}"
                                                                        target="_blank">
                                                                        <img src="{{ asset($files->image_path) }}"
                                                                            style="width: 50px;" alt="Image">
                                                                    </a>
                                                                    <span class="remove-btn" data-id="{{ $files->id }}"
                                                                        data-type="image">&times;</span>
                                                                </div>
                                                            @elseif($files->file_type == 'video')
                                                                {{-- <div class="media-wrapper" data-id="{{ $files->id }}">
                                          ...
                                          <span class="remove-btn" data-id="{{ $files->id }}" data-type="video">&times;</span>
                                                ...
                                                <td>{{ \Carbon\Carbon::parse($statement->created_on)->format('d M Y') }}
                                                </td>
                                                <td>{{ \Carbon\Carbon::parse($statement->updated_at)->format('d M Y') }}
                                                </td>
                                                <td>
                                                    <!-- Example Edit Button -->
                                                    <a href="{{ route('statements.edit', $statement->id) }}"
                                                    ...
                                                    <form action="{{ route('statements.destroy', $statement->id) }}"
                                                        ...
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" name="button" value="Delete Statement"
                            ...
                            <div class="card-footer clearfix">
                                <ul class="pagination pagination-sm m-0 float-right">
                                    ...
                                    @else
                                        <li class="page-item">
                                            <a class="page-link" href="{{ $statements->previousPageUrl() }}">&laquo;</a>
                                        </li>
                                    @endif

                                    {{-- Page numbers --}}
                                    @for ($i = 1; $i <= $statements->lastPage(); $i++)
                                        <li class="page-item {{ $i == $statements->currentPage() ? 'active' : '' }}">
                                            <a class="page-link" href="{{ $statements->url($i) }}">{{ $i }}</a>
                                        </li>
                                    @endfor
                                    ...
                                    @if ($statements->hasMorePages())
                                        <li class="page-item">
                                            <a class="page-link" href="{{ $statements->nextPageUrl() }}">&raquo;</a>
                                        </li>
                                    @else
                                        <li class="page-item disabled">
