@php
    ...
    $metaDescription =
        'Explore Elyvato - Seamlessly connect with vetted creatives - from editors to writers - exactly when you need them.';
...
@section('pageContent')
    <style>
        ...
        .modal-dialog {
            max-width: 405px;
            /* Adjusted for a better fit for the square image */
        }
            ...
            <div class="d-flex align-items-center flex-wrap gap-2 gap-lg-3 mb-4 mb-md-5">
                ...
                @foreach ($instanthire as $hire)
                    @if ($hire->is_available == 1)
                        <a href="{{ route('instant.hire.booking', $hire->slug) }}"
                            class="btn btn-filter border">{{ strtoupper($hire->name) }}</a>
                    @else
                        <a href="#" class="btn btn-filter border" data-bs-toggle="modal"
                            data-bs-target="#comingSoonModal">{{ strtoupper($hire->name) }}</a>
                    @endif
                @endforeach
            </div>
                    ...
                    <div class="col-md-6 col-lg-4 col-xl-3">
                        <div class="h-100 bg-white border border-bg-tertiary rounded service-card d-flex flex-column">
                            ...
                            <div class="service-card-image-box rounded-top">
                                @if ($hire->is_available == 1)
                                    <a href="{{ route('instant.hire.booking', $hire->slug) }}">
                                        <img src="{{ url($hire->image) }}" alt="{{ $hire->name }}"
                            ...
                            <div class="service-card-content p-4 d-flex flex-column flex-grow-1">
                                <div class="mb-4">
                                    <h3 class="fw-bold mb-3" style="word-wrap:break-word">
                                        @if ($hire->is_available == 1)
                                            <a href="{{ route('instant.hire.booking', $hire->slug) }}">
                                                @php
                                                    $name = $hire->name;
                                                    $splitIndex = strpos($name, ' ');
                                                    if ($splitIndex !== false) {
                                                        $firstPart = substr($name, 0, $splitIndex);
                                                        $secondPart = substr($name, $splitIndex + 1);
                                                    } else {
                                                        $firstPart = $name;
                                                        $secondPart = '';
                                                    }
                                ...
                                <div class="mt-auto">
                                    <a href="#" class="service-card-link d-inline-flex align-items-center"
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <img src="{{ url('front/assets/images/instant/commingsoon.png') }}" class="img-fluid"
                        style="border-radius: 8px;" alt="Coming Soon">
                </div>
            </div>
        </div>
    </div>
