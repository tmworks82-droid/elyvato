        ...
        <div class="container">
            ...
            <div class="row">
                <div class="col-lg-8 mb-4 mb-lg-0 pe-lg-5">
                                ...
                                <div class="item" data-hash="{{ $hash }}">
                                    <div class="ratio ratio-16x9 position-relative">
                                        ...
                                        @if ($thumb->file_type == 'image')
                                            <img src="{{ url($thumb->image_path) }}" alt="thumbnail"
                                                class="object-fit-cover">
                                        @elseif($thumb->file_type == 'video')
                                            <img src="https://img.youtube.com/vi/{{ $VideoId }}/hqdefault.jpg"
                                                alt="YouTube Thumbnail" class="object-fit-cover">
                                ...
                                <div class="col-2 col-sm-2 col-md-2 col-lg-2">
                                    <a href="#{{ $hash }}" class="ratio ratio-16x9 position-relative">
                                        ...
                                        @elseif($thumb->file_type == 'video')
                                            ...
                                            <div
                                                class="position-absolute top-0 left-0 w-100 h-100 d-flex align-items-center justify-content-center gig-detail-play-btn">
                                                <i class="ri-play-circle-line"></i>
                                            </div>
                                        @endif
                                    </a>
                    ...
                    {{-- for mobile read more visible  --}}
                    <div class="mb-3 mb-md-5  d-block d-md-none">
                        <div id="shortDesc">
                            {{ Str::words(strip_tags($sows->description), 30, '...') }}
                        </div>

                        <div id="fullDesc" class="d-none">
                            {!! $sows->description !!}
                        </div>

                        <a href="javascript:void(0);" class="text-primary d-block mt-2" id="toggleDesc">Read More</a>
                    </div>
                        ...
                        <div class="card mb-3 mb-lg-4">
                            <div class="card-body">
                                    ...
                                    @if (!empty($sows->min_price) && $sows->offer_price > 0)
                                        @php
                                            ...
                                            if (session('currency') == 'INR') {
                                                $original_price = $convertedPrice->max_price;
                                                $offer_price = $convertedPrice->offer_price;
                                            } else {
                                                $original_price = $convertedPrice->price; // maxprice
                                                $offer_price = $convertedPrice->offer_price;
                                            }
                                        ...
                                        @endphp
                                        <h4 class="fs-2 fw-semibold mb-3">{{ Currency_symbol(session('currency')) }}
                                            {{ $offer_price }}</h4>

                                        <small class="text-success"> <s>{{ Currency_symbol(session('currency')) }}
                                                {{ $original_price }}</s> {{ round($discount_percentage) }}% Off</small>
										<input type="hidden" id="_price" value="{{$offer_price}}">
                                    
									@else
                                        @php
                                            $convertedPrice = convertPrice($sows->id);
                                            if (session('currency') == 'INR') {
                                                $price = $convertedPrice->max_price;
                                            } else {
                                        ...
                                        <h4 class="fs-2 fw-semibold mb-3 ">{{ Currency_symbol(session('currency')) }}
                                            {{ $price }} </h4>
											<input type="hidden" id="_price" value="{{$price}}">
                                    @endif
                                ...
                                <div class="mt-3 pb-3">
                                    ...
                                    @if ($sows->is_subscription == 'yes')
                                        <p class="d-flex align-items-center gap-2"><i
                                                class="ri-vip-crown-2-line text-main"></i>{{ ucwords($sows->subscription_time) }}
                                            Subscription</p>
                                    @endif
                                </div>

                                <button type="button" class="btn btn-md-large btn-main w-100 d-flex align-items-center gap-2 justify-content-center"
                                    data-bs-toggle="modal" data-bs-target="#bookingModal"><i
                                        class="ri-phone-line"></i>Book
                                    Call</button>
                            </div>
                        </div>
                        <div class="card">
            ...
            <div class="row service-cards">
                ...
                @foreach (Service()->take(4) as $service)
                    <div class="col-md-6 col-lg-4 col-xl-3">
                        <div
                            class="h-100 bg-white border border-light-subtle rounded service-card shadow-sm d-flex flex-column">
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    @if (Config::get('services.payment.enabled') == true)
                        <p class="mb-0 text-accent">The ₹9 charge helps us ensure genuine interest and prevent spam. Don’t
                            worry it’s fully adjusted in your project cost.</p>
                    @endif
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" data-id="{{ $sows->id }}"
                        data-price="{{ $price = Config::get('services.payment.price') }}"
...
<!-- Mobile floating bar -->
<div id="mobileBookBar"
     class="position-fixed bottom-0 start-0 w-100 bg-white border-top shadow d-md-none"
     style="z-index:1049; display:none;">
  <div class="d-flex align-items-center justify-content-between gap-2 px-3 py-2">
    <div class="min-w-0">
      <div class="fw-semibold text-truncatee">{{ $sows->title }}</div>
      <small class="text-muted">
		
        {{ Currency_symbol(session('currency')) }}
        
		<span id="show_prices">0</span>
      </small>
    </div> <br>
    <button type="button" class="btn btn-main btn-sm px-3"
            data-bs-toggle="modal" data-bs-target="#bookingModal">
      <i class="ri-phone-line me-1"></i> Book Call
    </button>
  </div>
</div>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
			...
			let prices=$('#_price').val();
			
			$('#show_prices').text(prices);

            $(function() {
                $('#toggleDesc').on('click', function() {
                    $('#shortDesc, #fullDesc').toggleClass('d-none');
        ...
        $(document).on('click', '.proceed-booking-btn', function(e) {
            ...
            if (isLoggedIn == 0) {
                ...
                // Basic validation
                if (!email || !mobile) {
                    Swal.fire("Missing Info",
                        "Please complete your details by entering your email ID and contact information.",
                        "warning");
		...
		// here floating button js 

$(function () {
  const $bar = $('#mobileBookBar');
  const $sidebar = $('.sticky-top-lg-onav'); // your right sidebar wrapper

  const isMobile = () => window.matchMedia('(max-width: 767.98px)').matches;

  function sidebarInView() {
    if (!$sidebar.length) return false; // if no sidebar, show bar
    const r = $sidebar[0].getBoundingClientRect();
    return r.bottom > 0 && r.top < window.innerHeight; // any intersection
  }

  function updateBar() {
    if (!isMobile()) return $bar.hide();
    // show bar only when sidebar is OUT of view
    sidebarInView() ? $bar.stop(true, true).fadeOut(150)
                    : $bar.stop(true, true).fadeIn(150);
  }

  // Init + bind
  updateBar();
  $(window).on('scroll resize', updateBar);

  // Hide bar while modal is open (optional, keeps UI clean)
  $(document).on('show.bs.modal', '#bookingModal', () => $bar.hide());
  $(document).on('hidden.bs.modal', '#bookingModal', updateBar);
});
