        ...
        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
            <ol class="breadcrumb mb-0" itemscope itemtype="https://schema.org/BreadcrumbList">
              ...
              @if(!empty($subServiceSlug))
                <li class="breadcrumb-item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
                    <a href="https://elyvato.com/{{$serviceSlug}}/{{$subServiceSlug}}" class="breadcrumb-nlink" itemprop="item">
...
<section class="pt-4 section-padding-bottom">
	<div class="container">
        ...
        <div class="d-flex align-items-center flex-wrap gap-2 gap-lg-3 mb-4 mb-md-5">
			<div class="dropdown">
                <button class="btn btn-filter border dropdown-toggle" role="button" data-bs-toggle="dropdown" aria-expanded="false" data-bs-auto-close="outside">
                    Delivery Time 
                </button>
                <div class="dropdown-menu p-3 border shadow-sm service-filter-menu">
    <div class="form-check mb-1">
        <input class="form-check-input focus-shadow-none" type="radio" name="delivery_time" id="radioDefault1" value="2" {{ request('delivery_time') == '2' ? 'checked' : '' }}>
        <label class="form-check-label" for="radioDefault1">Up to 2 days</label>
    </div>
    <div class="form-check mb-1">
        <input class="form-check-input focus-shadow-none" type="radio" name="delivery_time" id="radioDefault2" value="3" {{ request('delivery_time') == '3' ? 'checked' : '' }}>
        <label class="form-check-label" for="radioDefault2">Up to 3 days</label>
    </div>
    <div class="form-check mb-1">
        <input class="form-check-input focus-shadow-none" type="radio" name="delivery_time" id="radioDefault3" value="5" {{ request('delivery_time') == '5' ? 'checked' : '' }}>
        <label class="form-check-label" for="radioDefault3">Up to 5 days</label>
    </div>
    <div class="form-check mb-1">
        <input class="form-check-input focus-shadow-none" type="radio" name="delivery_time" id="radioDefault4" value="6" {{ request('delivery_time') == '6' ? 'checked' : '' }}>
        <label class="form-check-label" for="radioDefault4">Up to 6 days</label>
    </div>
    <div class="form-check mb-1">
        <input class="form-check-input focus-shadow-none" type="radio" name="delivery_time" id="radioDefault5" value="7" {{ request('delivery_time') == '7' ? 'checked' : '' }}>
                    ...
                    @foreach ($sow->allFiles as $files)
                        ...
                        @elseif (!$fileShown && $files->file_type == 'video')
                            ...
                            <div class="service-card-image-box rounded-top">
                              ...
                              <img src="https://img.youtube.com/vi/{{ $VideoId }}/hqdefault.jpg" alt="YouTube Thumbnail"  class="img-fluid service-card-image">
                            </div>
                            @php $fileShown = true; @endphp
                        @endif
                    @endforeach
						...
						<div class="mb-4">
							...
							<h3 class="fw-bold fs-4 mb-3">
                                 ...
                                 @php 
                                    ...
                                    <a href="{{ route('sow.details.sub', [$serviceSlug, $subServiceSlug, $sow->slug]) }}">
                                        {{ $sow->title }}
                                    </a>
                                @else
                                    @php 
...
<script>
 ...
 const minRange = document.getElementById('minRange');
  ...
  function updateSlider() {
    ...
    sliderTrack.style.background = `linear-gradient(to right, #dee2e6 ${percent1}%, #f97a00 ${percent1}%, #f97a00 ${percent2}%, #dee2e6 ${percent2}%)`;
 ...
 document.getElementById('resetFilter').addEventListener('click', function (e) {
   ...
   $(document).ready(function () {
        $('#load-more-btn').on('click', function () {
            ...
            button.html('<span class="d-flex align-items-center gap-2">Loading... <i class="ri-loader-4-line ri-spin"></i></span>');
            ...
            $.ajax({
                ...
                error: function () {
                    button.html('<span class="d-flex align-items-center gap-2">Try Again <i class="ri-restart-line"></i></span>');
                    button.prop('disabled', false);
                }
            });
        });
